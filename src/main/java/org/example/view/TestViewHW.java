...
public class TestViewHW {
	...
	public static void main(String[] args) {
		while (true) {
			User currentUser = SessionManagerImpl.getInstance().getLoggedInUser();
			...
			if (currentUser == null) {
				...
				System.out.println("8. 팔로잉 리뷰 조회"); // ✅ 추가
				System.out.println("0. 종료");
			}
			System.out.print("선택: ");
	...
	private static void register() {
		...
		try {
			...
			if (registeredUser != null) {
				SessionManagerImpl.getInstance().setLoggedInUser(registeredUser);
	...
	private static void login() {
		...
		try {
			...
			if (user != null) {
				SessionManagerImpl.getInstance().setLoggedInUser(user);
	...
	private static void showFollowingList() {
		...
		while (true) {
			...
			for (int i = start; i < end; i++) {
				...
				System.out.println("- " + u.getUserNo() + " | " + u.getName() + " (" + u.getEmail() + ")");
			}
	...
	private static void showFollowingReviews() {
		...
		while (true) {
			...
			for (int i = start; i < end; i++) {
				...
				System.out.println("- 작성자: " + r.getUserName() + " | 영화: " + r.getMovieName() + " | 평점: "
						+ r.getRating() + " | 좋아요: " + r.getLikeCount() + " | 리뷰: " + r.getContent() + " | 작성일: "
						+ r.getRegDate());
			}
			System.out.println("\n[n] 다음 페이지 | [p] 이전 페이지 | [q] 나가기");
			String cmd = sc.next();
